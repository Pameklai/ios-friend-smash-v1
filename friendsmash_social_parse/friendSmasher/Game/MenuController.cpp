...
namespace FriendSmasher
{
    namespace Game
    {
        ...
        static void didFetchPropertyFromParse(NSString* property, u32 uValue) {
            if (kpMenuController) {
                kpMenuController->DidFetchPropertyFromParse(property, uValue);
            }
        }
        
        static void didSavePropertyToParse(NSString* property, bool bSucceeded) {
            if (kpMenuController) {
                kpMenuController->DidSavePropertyToParse(property, bSucceeded);
            }
        }
        
        
        u32 MenuController::ms_uBombs = 0;
        u32 MenuController::ms_uCoins = 0;
...
//                FacebookController::Login(didLogIn);
        ...
        void MenuController::ShowEndGame(NSString* friendName, u64 uFriendID, u32 uScore, u32 uCoinsWon, u32 uBombsRemaining)
        {
            ...
            ParseController::SetProperty(@"bombs", ms_uBombs, didSavePropertyToParse);
            ParseController::SetProperty(@"coins", ms_uCoins, didSavePropertyToParse);
        }
...
#endif
                if (!m_bReturningFromGame) {
                    ParseController::GetProperty(@"bombs", didFetchPropertyFromParse);
                    ParseController::GetProperty(@"coins", didFetchPropertyFromParse);
                }
        ...
        void MenuController::DidFetchPropertyFromParse(NSString* property, u32 uValue)
        {
            if ([property isEqualToString: @"bombs"])
            {
                ms_uBombs = uValue;
                m_labelBombs.text = [NSString stringWithFormat:@"%d", ms_uBombs];
            }
            else if ([property isEqualToString: @"coins"])
            {
                ms_uCoins = uValue;
                m_labelCoins.text = [NSString stringWithFormat:@"%d", ms_uCoins];
            }
        ...
        void MenuController::DidSavePropertyToParse(NSString* property, bool bSucceeded)
        {
            bSucceeded ? NSLog(@"Save to Parse succeeded") : NSLog(@"Save to Parse failed");
